...
import { C, ACTIVITY_STAT_MAP } from "./constants";
import { xpForLevel, totalXpForLevel, distributeStatPoints, processLevelUp } from "./gameLogic";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      if (!session) { setScreen("landing"); setUserId(null); }
    });
  ...
  const handleSignOut = async () => {
    ...
    setScreen("landing"); setUserId(null);
    setPlayerName("Adventurer"); setPlayerClass("warrior");
    ...
    setCompletedRituals({}); setCompletedQuests([]);
    setUserGuild(null); setGuildMembers([]);
    setAvatarUrl(null);
  ...
  const handleRitualComplete = async (ritualName) => {
    ...
    if (userId) {
      const today = new Date().toISOString().split("T")[0];
      const ritualColumn = {
        "Bodyweight Workout": "bodyweight_workout", "Walk/Jog 20min": "walk_jog",
        "Read 20min": "read_20", "Pray/Meditate 10min": "pray_meditate", "Reach Out": "reach_out",
      }[ritualName];
      if (ritualColumn) {
      ...
      saveProfile({ xp: newXP, gold: newGold, [`tally_${stat}`]: newTally[stat] });
    }
  };
  ...
  const handleCreateGuild = async (name, description) => {
    ...
    } catch (e) { console.error("Failed to create guild:", e); }
  };
  ...
  const handleJoinByCode = async (code) => {
    ...
    try {
      ...
      setGuildMembers(members || []);
    } catch (e) { console.error("Failed to join guild:", e); }
  };
